...
export const usePOSCartStore = defineStore("posCart", () => {
	...
	async function updateItemDetails(itemCode, updates, currentUom = null) {
		try {
			console.log('[updateItemDetails] Called with itemCode:', itemCode, 'updates:', updates, 'currentUom:', currentUom)
			const cartItem = findCartItem(itemCode, currentUom)
			if (!cartItem) {
				throw new Error("Item not found in cart")
			}
			console.log('[updateItemDetails] Found cartItem:', cartItem.item_name, 'current rate:', cartItem.rate, 'price_list_rate:', cartItem.price_list_rate)
			...
			recalculateItem(cartItem)
			console.log('[updateItemDetails] After recalculate - rate:', cartItem.rate, 'price_list_rate:', cartItem.price_list_rate, 'amount:', cartItem.amount)

			// Force reactivity by creating new object reference if rate was changed
			if (updates.rate !== undefined) {
				const itemIndex = invoiceItems.value.findIndex(i => i === cartItem)
				if (itemIndex >= 0) {
					invoiceItems.value[itemIndex] = { ...cartItem }
					console.log('[updateItemDetails] Forced reactivity update at index', itemIndex)
				}
			}
