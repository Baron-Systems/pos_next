...
export const useCustomerSearchStore = defineStore("customerSearch", () => {
	...
	const favoriteCustomer = ref(null)
	...
	function quickMatch(search, customer) {
		...
		if (!cached) {
			cached = {
				...
				idNo: (customer.id_no || "").toLowerCase(),
		...
		// ID Number (id_no) checks
		if (cached.idNo === term) return 130
		if (cached.idNo.startsWith(term)) return 125
		if (cached.idNo.includes(term)) return 85
	...
	const filteredCustomers = computed(() => {
		...
		// Search all customers - no limit
		const results = []

		// Get all matching customers with scores
		...
		// Sort by score (highest first) and map to customers
	...
	function loadCustomerHistory() {
		try {
			...
			const favorite = localStorage.getItem("pos_favorite_customer")

			if (recent) recentSearches.value = JSON.parse(recent)
			if (frequent) frequentCustomers.value = JSON.parse(frequent)
			if (favorite) favoriteCustomer.value = JSON.parse(favorite)
		} catch (e) {
	...
	/**
	 * Set a customer as the favorite (default) customer.
	 * Only one customer can be favorite at a time.
	 * If the same customer is passed, it will be toggled off.
	 * @param {string} customerId - The customer name/ID to set as favorite
	 */
	function setFavoriteCustomer(customerId) {
		if (!customerId) {
			favoriteCustomer.value = null
		} else if (favoriteCustomer.value === customerId) {
			// Toggle off if same customer
			favoriteCustomer.value = null
		} else {
			// Set new favorite (only one allowed)
			favoriteCustomer.value = customerId
		}

		// Persist to localStorage
		try {
			if (favoriteCustomer.value) {
				localStorage.setItem(
					"pos_favorite_customer",
					JSON.stringify(favoriteCustomer.value),
				)
			} else {
				localStorage.removeItem("pos_favorite_customer")
			}
		} catch (e) {
			log.warn("Failed to persist favorite customer:", e)
		}
	}

	/**
	 * Toggle favorite status for a customer.
	 * If already favorite, removes it. Otherwise sets it as favorite.
	 * @param {string} customerId - The customer name/ID to toggle
	 */
	function toggleFavoriteCustomer(customerId) {
		if (!customerId) return
		setFavoriteCustomer(favoriteCustomer.value === customerId ? null : customerId)
	}

	/**
	 * Check if a customer is the current favorite.
	 * @param {string} customerId - The customer name/ID to check
	 * @returns {boolean}
	 */
	function isFavoriteCustomer(customerId) {
		return favoriteCustomer.value === customerId
	}

	/**
	 * Get the favorite customer object from the allCustomers list.
	 * @returns {Object|null} The favorite customer object or null
	 */
	const favoriteCustomerObject = computed(() => {
		if (!favoriteCustomer.value) return null
		return allCustomers.value.find(c => c.name === favoriteCustomer.value) || null
	})
