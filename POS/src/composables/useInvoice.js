...
export function useInvoice() {
	...
	function addItem(item, quantity = 1) {
		...
		const existingItem = existingItemIndex > -1 ? invoiceItems.value[existingItemIndex] : null

		if (existingItem) {
			// Store old values before update for incremental cache adjustment
			// Use rate for subtotal calculations (respects manually edited prices)
			const oldPriceListRate = existingItem.rate || existingItem.price_list_rate
			...
			invoiceItems.value.splice(existingItemIndex, 1)
			invoiceItems.value.unshift(existingItem)

			// Update cache incrementally (new values - old values)
			// Use rate for subtotal (respects manually edited prices)
			const effectiveRate = existingItem.rate || existingItem.price_list_rate
			_cachedSubtotal.value +=
				existingItem.quantity * effectiveRate - oldAmount
			...
			// Use rate for subtotal (respects manually edited prices)
			const effectiveRate = newItem.rate || newItem.price_list_rate
			_cachedSubtotal.value += newItem.quantity * effectiveRate
	...
	function removeItem(itemCode, uom = null) {
		...
		if (itemToRemove) {
			...
			// Use rate for subtotal (respects manually edited prices)
			const effectiveRate = itemToRemove.rate || itemToRemove.price_list_rate
	...
	function updateItemQuantity(itemCode, quantity, uom = null, moveToTop = true) {
		...
		if (uom) {
			...
			itemIndex = invoiceItems.value.findIndex((i) => i.item_code === itemCode)
		}
		...
		if (itemIndex > -1) {
			...
			// Store old values before update for incremental cache adjustment
			// Use rate for subtotal calculations (respects manually edited prices)
			const oldPriceListRate = item.rate || item.price_list_rate
			...
			// Update cache incrementally (new values - old values)
			// Use rate for subtotal (respects manually edited prices)
			const effectiveRate = item.rate || item.price_list_rate
	...
	function updateItemRate(itemCode, rate) {
		const itemIndex = invoiceItems.value.findIndex((i) => i.item_code === itemCode)
		if (itemIndex > -1) {
			const item = invoiceItems.value[itemIndex]
			// Store old values before update for incremental cache adjustment
			// Use rate for subtotal calculations (respects manually edited prices)
	...
	function rebuildIncrementalCache() {
		...
		for (const item of invoiceItems.value) {
			// Use rate directly for subtotal (respects manually edited prices)
			// price_list_rate is kept for reference but rate is the actual selling price
	...
	function computeBackendRate(item) {
		...
		if (taxInclusive.value) {
			// Inclusive: Start from effective rate, subtract discount portion
			return effectiveRate - (discountAmount / qty)
		}
	...
	function formatItemsForSubmission(items) {
		return items.map((item) => ({
			...
			price_list_rate: item.rate || item.price_list_rate,
			uom: item.uom,
			warehouse: item.warehouse,
			batch_no: item.batch_no,
			serial_no: item.serial_no,
			...
			// Sales Order linking fields - critical for updating sales order status
			sales_order: item.against_sales_order || null,  // Map to correct ERPNext field
			so_detail: item.so_detail || null,
		}))
	}
	 ...
	 * Favorite customer is handled separately by POSSale.vue to avoid circular deps.
	...
	async function setDefaultCustomer() {
		...
		// Check if there's a favorite customer in localStorage
		// If yes, skip setting default - POSSale.vue will handle it
		try {
			...
			if (favoriteCustomerId) {
				...
				if (favoriteId) {
					// Don't set customer here - let POSSale.vue handle it after stores load
					// This avoids circular dependency issues
					return
				}
			}
		} catch (error) {
			// Continue to POS Profile default
		}

		// Fallback: Get default customer from POS Profile
		try {
	...
	async function resetInvoice() {
		...
		// Set default customer from favorite or POS Profile if available
