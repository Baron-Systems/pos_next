			...
			<div class="flex flex-col gap-1.5">
				<div class="flex gap-1.5 min-h-[40px]">
					...
					>
						...
						<span dir="ltr" class="text-white whitespace-nowrap">{{ __("Pay", null, "payment") }}</span>
					</button>
					<!-- Quick Print and Pay (Cash + print receipt) -->
					<button
						type="button"
						@click="$emit('quick-cash-print-pay')"
						:disabled="items.length === 0"
						:style="items.length === 0 ? { backgroundColor: '#d1d5db', color: '#6b7280' } : { backgroundColor: '#059669', color: '#fff' }"
						:class="[
							'flex-1 min-w-[6.5rem] py-2.5 px-2 rounded-lg font-bold text-xs transition-all inline-flex items-center justify-center gap-1.5 touch-manipulation overflow-visible',
							items.length === 0
								? 'bg-gray-300 text-gray-500 cursor-not-allowed'
								: 'bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white shadow active:scale-[0.98]',
						]"
						:title="__('Pay with cash and print receipt')"
						:aria-label="__('Print and Pay')"
					>
						<svg class="w-4 h-4 flex-shrink-0 text-inherit" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
							<path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h12a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h2z"/>
						</svg>
						<span dir="ltr" class="whitespace-nowrap text-inherit">{{ __("Print and Pay") }}</span>
					</button>
				</div>
				<div class="flex gap-1.5 min-h-[40px]">
					<!-- Checkout Button (opens payment dialog) -->
					<button
						type="button"
						@click="handleProceedToPayment"
						:disabled="items.length === 0"
						:class="[
							'flex-1 min-w-0 py-2.5 px-3 rounded-lg font-bold text-xs transition-all inline-flex items-center justify-center gap-1.5 touch-manipulation overflow-visible',
							items.length === 0
								? 'bg-gray-300 text-gray-500 cursor-not-allowed'
					...
					>
						...
						<span dir="ltr" class="text-white whitespace-nowrap">{{ __("Checkout") }}</span>
					</button>
...
const emit = defineEmits([
	...
	"quick-cash-print-pay", // () - Pay with cash and print receipt
