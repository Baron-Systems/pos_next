				...
				<div class="flex flex-1 gap-1 sm:gap-2 min-w-0">
					...
					<div v-if="!compact" class="relative flex-1 min-w-0">
						...
						<input
							...
							:class="[
								'w-full text-[11px] sm:text-sm border rounded-lg px-2 sm:px-3 py-2 ps-7 sm:ps-9 pe-8 focus:outline-none transition-all',
						...
						<button
							...
							type="button"
							@click.stop="clearManualSearch"
							class="absolute inset-y-0 end-1 flex items-center justify-center w-6 h-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors touch-manipulation"
					...
					<!-- Barcode input: hidden on mobile, shown on larger screens -->
					<div class="relative w-28 sm:w-36 flex-shrink-0 hidden sm:block">
						...
						<input
							...
							@blur="handleSearchBlur"
							class="w-full text-[11px] sm:text-sm border-2 border-green-400 bg-green-50 rounded-lg px-2 py-2 ps-7 pe-2 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
							:aria-label="__('Barcode scan')"
						/>
					</div>
				</div>
				...
				<div v-if="!compact" class="flex items-center gap-0.5 flex-shrink-0">
					<button
						...
						:class="[
							...
							scannerEnabled ? 'bg-green-100 hover:bg-green-200 text-green-700' : 'hover:bg-gray-100 text-gray-600'
						]"
						:title="scannerEnabled ? __('Barcode: ON') : __('Barcode: OFF')"
						:aria-label="scannerEnabled ? __('Disable barcode') : __('Enable barcode')"
					>
						<svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/>
						</svg>
					...
					<button
						@click="toggleAutoAdd"
						:class="[
							'p-1 sm:p-1.5 rounded flex items-center gap-0.5 text-[9px] sm:text-xs font-medium px-1 sm:px-2 touch-manipulation',
							autoAddEnabled ? 'bg-blue-100 hover:bg-blue-200 text-blue-700' : 'hover:bg-gray-100 text-gray-600'
						]"
						:title="autoAddEnabled ? __('Auto-Add: ON (manual search)') : __('Auto-Add: OFF')"
		...
		<div v-if="viewMode === 'grid'" key="grid" class="flex-1 flex flex-col overflow-hidden min-h-0">
			...
			>
				<div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1.5 sm:gap-2.5">
					...
					>
						<!-- Favorite (pin) - small heart, does not add to cart -->
						<button
							type="button"
							@click.stop="toggleFavorite(item.item_code)"
							:class="[
								'absolute -top-1.5 -start-1.5 sm:-top-2 sm:-start-2 rounded-full w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center z-20 border-2 border-white shadow-md transition-all touch-manipulation',
								isFavorite(item.item_code)
									? 'bg-red-500 text-white hover:bg-red-600 active:scale-95'
									: 'bg-white/90 text-gray-400 hover:text-red-400 hover:bg-white',
							]"
							:title="isFavorite(item.item_code) ? __('Unpin from favorites') : __('Pin as favorite / frequently used')"
							:aria-label="isFavorite(item.item_code) ? __('Unpin') : __('Pin as favorite')"
						>
							<svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" :class="isFavorite(item.item_code) ? 'fill-current' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
						...
						>
							{{ (item.actual_qty ?? item.stock_qty ?? 0) }}
						</div>
							...
							<td class="px-2 sm:px-3 py-2 max-w-[120px] sm:max-w-[180px] md:max-w-[200px]">
								<div class="flex items-center gap-1 min-w-0">
									<button
										...
										@click.stop="toggleFavorite(item.item_code)"
										:class="[
											'flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all touch-manipulation',
											isFavorite(item.item_code)
												? 'text-red-500 hover:text-red-600 bg-red-50'
												: 'text-gray-400 hover:text-red-400 hover:bg-gray-100',
										]"
										:title="isFavorite(item.item_code) ? __('Unpin from favorites') : __('Pin as favorite')"
										:aria-label="isFavorite(item.item_code) ? __('Unpin') : __('Pin as favorite')"
									>
										<svg class="w-3.5 h-3.5" :class="isFavorite(item.item_code) ? 'fill-current' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
										</svg>
									</button>
									<div class="text-xs sm:text-sm font-medium text-gray-900 truncate min-w-0" :title="item.item_name">
										{{ item.item_name }}
									</div>
								</div>
							</td>
...
const { isFavorite, toggleFavorite, favoriteItemCodes } = useFavoriteItems()
...
// Barcode-only input: always ready for scanner, clears automatically after scan
...
// Sorted so favorites (pinned) appear first
const sortedFilteredItems = computed(() => {
	const list = filteredItems.value || []
	const fav = favoriteItemCodes.value
	if (!fav || typeof fav.has !== "function") return list
	return [...list].sort((a, b) => (fav.has(b.item_code) ? 1 : 0) - (fav.has(a.item_code) ? 1 : 0))
})

// Computed paginated items (uses sorted list so favorites show first)
const paginatedItems = computed(() => {
	if (!sortedFilteredItems.value.length) return []
...
const totalPages = computed(() => {
	if (!sortedFilteredItems.value.length) return 0
	return Math.ceil(sortedFilteredItems.value.length / itemsPerPage.value)
})
...
const BARCODE_PLACEHOLDER = __("Scan barcode")
const MANUAL_PLACEHOLDER = __("Search by name or code")
...
onMounted(() => {
	...
	// Keep barcode input focused so scanner is always ready
	nextTick(() => {
...
async function handleBarcodeKeyDown(event) {
	...
	if (
		...
		scannerInputDetected.value = true
...
// Manual search: filter only, no add to cart on Enter (user adds by clicking item from list)
function handleManualKeyDown(event) {
	if (event.key === "Enter") {
		event.preventDefault()
		// Do not add to cart from manual search — only filtering
	}
}

// Handle search input: filter list only (no auto-add to cart)
function handleSearchInput(event) {
	const value = event.target.value
	itemStore.setSearchTerm(value)
	// No timer, no auto-add — manual search is for filtering only; user adds by clicking item
...
// Manual search: click does not clear (user is typing; clear only via backspace or clear icon)
function handleSearchClick() {
	// Do not auto-clear manual search - user writes there
}

function clearManualSearch() {
	itemStore.clearSearch()
	nextTick(() => {
		const input = searchInputRef.value || document.getElementById("item-search-manual")
		if (input) input.focus()
	})
}

// Keep barcode ready for scanner: refocus only when focus left to non-input (e.g. middle of screen)
// Never steal focus from the manual search input so user can type there
const SEARCH_REFOCUS_DELAY_MS = 120

function handleSearchBlur(event) {
	const relatedTarget = event?.relatedTarget
	const manualInput = searchInputRef.value || document.getElementById("item-search-manual")
	// If focus is moving to the manual search input, do nothing - let user type there
	if (relatedTarget && manualInput && (relatedTarget === manualInput || manualInput.contains(relatedTarget))) return

	setTimeout(() => {
		const barcodeInput = barcodeInputRef.value || document.getElementById("item-search")
		if (!barcodeInput) return
		const active = document.activeElement
		if (active === barcodeInput) return
		// Don't steal focus from manual search or any input/button/dialog
		if (active === manualInput) return
...
/**
 * Run barcode/search and optionally add to cart.
 * @param {string} value - Barcode or search term
 * @param {boolean} forceAutoAdd - Add to cart on single match
 * @param {{ clearBarcode?: boolean, clearManual?: boolean }} options - clearBarcode: clear barcode input after (scanner). clearManual: clear store search (manual) after.
 */
async function handleBarcodeSearchWithValue(value, forceAutoAdd = false, options = {}) {
	const { clearBarcode = false, clearManual = false } = options
	...
	// When from barcode field → focus barcode. When from manual search → keep focus in manual search (don’t leave the box).
	...
	if (filteredItems.value.length === 0) {
		...
		if (clearBarcode) barcodeInputValue.value = ""
		if (clearManual) itemStore.clearSearch()
		// From manual search: keep focus in search box even when item not found
		focusAfterAction()
	} else {
		if (shouldAutoAdd) {
