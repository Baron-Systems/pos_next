...
watch(show, (newVal) => {
	if (newVal) {
		...
		customAmount.value = ""
		// Default amount = grand total so user doesn't have to type it every time
		const defaultAmount = round2(props.grandTotal)
		if (defaultAmount > 0) {
			numpadDisplay.value = defaultAmount.toFixed(2)
			mobileCustomAmount.value = defaultAmount.toFixed(2)
		} else {
			numpadDisplay.value = ""
			mobileCustomAmount.value = ""
		}
...
function addCustomPayment(method, amount) {
	...
	// Pre-fill amount with remaining total for next payment (so user doesn't type every time)
	nextTick(() => {
		const remaining = remainingAmount.value
		if (remaining > 0) {
			numpadDisplay.value = round2(remaining).toFixed(2)
			mobileCustomAmount.value = round2(remaining).toFixed(2)
		} else {
			numpadDisplay.value = ""
			mobileCustomAmount.value = ""
		}
