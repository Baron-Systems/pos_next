<template>
	<Dialog
		...
		:options="{ title: __('تسجيل مصروف الوردية'), size: '3xl' }"
	>
		<template #body-content>
			<div class="flex flex-col gap-5 py-2 min-h-[50vh]">
				<!-- Payment Method -->
				<div class="flex flex-col gap-1">
					<label class="text-base font-medium text-gray-700">{{ __('طريقة الدفع') }}</label>
					<select
				...
				<!-- Expense Name -->
				<div class="flex flex-col gap-1">
					<label class="text-base font-medium text-gray-700">{{ __('اسم المصروف') }}</label>
					<select
