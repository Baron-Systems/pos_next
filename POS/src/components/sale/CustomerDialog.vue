<template>
	<Dialog v-model="show" :options="{ title: __('Select Customer'), size: 'lg' }">
		<template #body-title>
		...
		<template #body-content>
			<div class="flex flex-col gap-4">
				...
				<p v-if="!loading && allCustomers.length > 0" class="text-start text-xs text-gray-500 mt-0">
					<span v-if="showingRecent" class="text-blue-600 font-medium">⭐ {{ __('All Customers ({0})', [allCustomers.length]) }}</span>
					...
					<div v-else class="flex flex-col gap-2">
						...
						>
							<div class="flex items-start justify-between">
								...
								<div class="ms-2 flex-shrink-0 flex items-center gap-2">
									<!-- Favorite Heart Button -->
									<button
										type="button"
										@click.stop="toggleFavoriteCustomer(customer.name)"
										:class="[
											'w-8 h-8 rounded-full flex items-center justify-center transition-all duration-150',
											isFavoriteCustomer(customer.name)
												? 'bg-red-50 text-red-500 hover:bg-red-100'
												: 'bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-500'
										]"
										:title="isFavoriteCustomer(customer.name) ? __('Remove from favorites') : __('Set as favorite customer')"
									>
										<svg
											class="w-5 h-5"
											:fill="isFavoriteCustomer(customer.name) ? 'currentColor' : 'none'"
											stroke="currentColor"
											viewBox="0 0 24 24"
										>
											<path
...
// Favorite customer helpers
const { toggleFavoriteCustomer, isFavoriteCustomer } = customerStore
