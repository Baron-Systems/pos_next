<template>
	<Dialog v-model="show" :options="{ title: isEditMode ? 'تعديل المورد' : 'إنشاء مورد جديد', size: 'xl' }">
		<template #body-content>
			<div class="flex flex-col gap-6 min-h-[400px]">
				<!-- Supplier Name (Required) -->
				<div>
					<label class="block text-start text-sm font-medium text-gray-700 mb-2">
						{{ __("اسم المورد") }} <span class="text-red-500">*</span>
					</label>
					<Input
				...
				<div>
					...
					>
						<option value="">{{ __("Select Supplier Group") }}</option>
			...
			<div class="flex flex-col gap-2">
					...
					<div class="flex items-start gap-2">
						...
						<div class="flex-1">
							...
							<p class="text-xs text-amber-700 mt-0.5">
								{{ __("You don't have permission to create suppliers. Contact your administrator.") }}
							</p>
						</div>
					</div>
				</div>
				...
				<div class="flex gap-2">
					<Button
						variant="solid"
						@click="handleSubmit"
						:loading="createSupplierResource.loading || checkingPermission"
						:disabled="!customerData.customer_name || !hasPermission"
					>
						{{ isEditMode ? __("Save Changes") : __("Create Supplier") }}
					</Button>
...
const log = logger.create("CreateSupplierDialog")
...
const emit = defineEmits(["update:modelValue", "supplier-created"])
...
const customerGroups = ref(["All Supplier Groups"])
...
const isEditMode = computed(() => !!props.supplier?.name)
...
const loadSupplierData = async (supplier) => {
	if (!supplier?.name) {
		log.warn("No supplier data provided for edit mode")
		return
	}

	isLoadingCustomerData.value = true
	log.info(`Loading supplier data for: ${supplier.name}`, supplier)

	try {
		// Fetch fresh supplier data
		let freshSupplier = supplier
		try {
			...
			if (result) {
				freshSupplier = result
				log.info(`Fetched fresh supplier data:`, freshSupplier)
			}
		} catch (fetchError) {
			log.warn(`Could not fetch fresh supplier data, using cached: ${fetchError.message}`)
		}

		// Set basic supplier info from fresh data
		customerData.value.customer_name = freshSupplier.supplier_name || freshSupplier.name || ""
		customerData.value.email_id = freshSupplier.email_id || ""
		customerData.value.customer_group = freshSupplier.supplier_group || ""
		customerData.value.territory = freshSupplier.territory || ""

		// Handle phone number parsing from fresh data
		const mobileNo = freshSupplier.mobile_no || ""
		...
		log.info(`Supplier data loaded successfully for: ${supplier.name}`)
	} catch (error) {
		log.error("Error loading supplier data:", error)
		showError(__("Error loading supplier data"))
	} finally {
 ...
 */
const createSupplierResource = createResource({
	url: "pos_next.api.supplier.create_supplier",
	makeParams: () => {
		const params = {
			supplier_name: customerData.value.customer_name,
			mobile_no: customerData.value.mobile_no,
			email_id: customerData.value.email_id || "",
			supplier_group: customerData.value.customer_group || undefined,
			supplier_type: "Company",
		}
		log.info("Create supplier params:", params)
		return params
	},
	onSuccess: (data) => {
		log.info("Supplier created successfully:", data)
		showSuccess(__("Supplier {0} created successfully", [data.supplier_name || data.name]))
...
const customerGroupsResource = createListResource("Supplier Group", (names) => (customerGroups.value = names))
...
const loadDialogData = async () => {
	...
	if (isEditMode.value && props.supplier) {
		// In edit mode, supplier data loading is handled by loadSupplierData
		await loadSupplierData(props.supplier)
...
const handleSubmit = async () => {
	...
	if (!customerData.value.customer_name) {
		return showError(__("اسم المورد مطلوب"))
	}
	...
	try {
		await createSupplierResource.submit()
	} catch (error) {
...
// Watch supplier prop changes (edit mode) - immediate to load data when dialog opens with supplier
watch(
	() => props.supplier,
	(newSupplier, oldSupplier) => {
		// Only reload if supplier actually changed
		if (newSupplier?.name && newSupplier.name !== oldSupplier?.name) {
			log.info(`Supplier prop changed: ${newSupplier.name}`)
