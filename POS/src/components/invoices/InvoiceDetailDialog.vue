				...
				<div class="flex items-center gap-2">
					<Button
						...
						:class="[
							...
							customerWhatsApp ? 'hover:bg-green-50' : 'opacity-50 cursor-not-allowed',
						]"
...
const originalInvoiceData = ref(null) // For return invoices - stores the original invoice
...
// Computed: Check if this is a credit sale return (checks original invoice payments)
const isCreditSaleReturn = computed(() => {
	if (!invoiceData.value || !invoiceData.value.is_return) return false
	// For returns, check the ORIGINAL invoice to determine if it was a credit sale
	// If original invoice data is available, use it; otherwise fall back to current logic
	const sourceInvoice = originalInvoiceData.value || invoiceData.value
	const hasNoPayments = !sourceInvoice.payments || sourceInvoice.payments.length === 0
	const totalPaid = sourceInvoice.payments?.reduce((sum, p) => sum + Math.abs(p.amount || 0), 0) || 0
	const grandTotal = Math.abs(sourceInvoice.grand_total || 0)
	// Credit sale if original invoice had no payments or very small payment
	return hasNoPayments || (totalPaid < 0.01 && grandTotal > 0.01)
})

// Computed: Customer WhatsApp/mobile number, normalized for wa.me.
// Uses the same convention as CustomerPaymentDialog: keep digits/+, default to +970.
const customerWhatsApp = computed(() => {
	if (!invoiceData.value) return ""
	const raw = invoiceData.value.customer_mobile_no || invoiceData.value.customer_mobile || ""
	if (!raw) return ""
...
async function loadInvoiceDetails() {
	...
	loading.value = true
	originalInvoiceData.value = null // Reset original invoice data
	try {
		...
		// If this is a return invoice, also load the original invoice to check payment status
		if (result && result.is_return && result.return_against) {
			try {
				const originalResult = await call("pos_next.api.invoices.get_invoice", {
					invoice_name: result.return_against,
				})
				originalInvoiceData.value = originalResult
			} catch (originalError) {
				log.warn("Could not load original invoice details:", originalError)
				// Don't fail the whole operation if original invoice can't be loaded
			}
		}
	} catch (error) {
		log.error("Error loading invoice details:", error)
		invoiceData.value = null
		originalInvoiceData.value = null
	} finally {
		loading.value = false
	}
}
...
function handleWhatsApp() {
	...
	if (!customerWhatsApp.value) {
		showWarning(__("This customer has no stored WhatsApp/mobile number."))
		return
	}

	const companyName = invoiceData.value.company || ""
	const customerName = invoiceData.value.customer_name || invoiceData.value.customer || ""
	const invoiceName = invoiceData.value.name || ""
	const invoiceDate = formatDate(invoiceData.value.posting_date)
	const grandTotal = formatCurrency(invoiceData.value.grand_total || 0)

	const itemsText = (invoiceData.value.items || [])
		.map((item) => {
			const qty = item.quantity || item.qty || 1
			const amount = formatCurrency(item.amount || 0)
			return `• ${item.item_name} × ${qty} = ${amount}`
		})
		.join("\n")

	const messageLines = [
		companyName ? `${companyName} ترحب بكم` : "مرحباً بكم",
		`مرحباً ${customerName}`,
		"",
		"تفاصيل فاتورتك:",
	...
	const url = `https://wa.me/${customerWhatsApp.value}?text=${encodeURIComponent(message)}`
