	...
	>
		<div class="flex py-2 sm:py-3">
			...
			<div class="w-16 flex-shrink-0 flex items-center justify-center">
				<button
					class="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-md flex-shrink-0 hover:bg-gray-50 active:scale-95 transition-all"
					:aria-label="'POS-ABS'"
					:title="__('POS-ABS')"
				>
					<img src="/icon.svg" class="w-6 h-6" alt="POS-ABS" />
				</button>
			</div>
			...
			<div class="flex-1 flex justify-between items-center gap-1 sm:gap-2 px-2 sm:px-4 md:px-6">
				...
				<div class="flex items-center gap-1 sm:gap-4 min-w-0 flex-1 overflow-hidden">
					<div class="min-w-0 flex-shrink overflow-hidden">
						<div class="flex items-center gap-1 sm:gap-2">
							<h1 class="text-xs sm:text-base font-bold text-gray-900 truncate flex-shrink">{{ 'POS-ABS' }}</h1>
				...
				<div class="flex items-center gap-0.5 sm:gap-1 md:gap-2 flex-shrink-0">
					...
					<button
						@click="emit('toggle-success-dialog')"
						:class="[
							'p-1.5 sm:p-2 hover:bg-gray-100 active:bg-gray-200 rounded-lg transition-colors relative group touch-manipulation',
							showSuccessDialog ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'
						]"
						:title="showSuccessDialog ? __('Show success dialog after invoice') : __('Hide success dialog after invoice')"
						:aria-label="showSuccessDialog ? __('Success dialog enabled') : __('Success dialog disabled')"
					>
