...
const linkText = ref('Al Baron System')
const footerLink = ref('https://albaronsystems.com')
...
const loadBrandingConfig = async () => {
	try {
		...
		console.error('[Al Baron System] Failed to load branding config:', error)
		// Use fallback values
		footerText.value = 'Powered by'
		linkText.value = 'Al Baron System'
		footerLink.value = 'https://albaronsystems.com'
	}
}
...
const logClientEvent = async (eventType, details = {}) => {
	try {
		...
		console.error('[Al Baron System] Failed to log event:', error)
	}
}
...
const ensureBranding = () => {
	...
	const expectedBrand = atob(config.value._l || btoa('Al Baron System'))
	const expectedUrl = atob(config.value._u || btoa('https://albaronsystems.com'))
