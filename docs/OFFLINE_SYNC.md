...
This document describes the offline invoice synchronization system in POS-ABS, including the deduplication mechanism that prevents duplicate invoices from being created during network failures.

## Overview

POS-ABS supports fully offline operation, allowing cashiers to continue creating invoices even when the network connection is unavailable. When connectivity is restored, pending invoices are automatically synchronized to the server.
