...
def update_invoice(data):
    ...
    try:
        ...
        # Store original payments to preserve them (ERPNext may reset during validation)
        original_payments = data.get('payments', [])
        ...
        # Restore payments if ERPNext reset them during document creation (handle both regular and returns)
        payments_restored = False
        if original_payments and invoice_doc.get('payments'):
            for i, payment in enumerate(invoice_doc.payments):
                if i < len(original_payments):
        ...
        # Restore payments again if ERPNext reset them during save/validation (handle both regular and returns)
        payments_restored_again = False
        if original_payments and invoice_doc.get('payments'):
            for i, payment in enumerate(invoice_doc.payments):
                if i < len(original_payments):
                    original = original_payments[i]
...
def submit_invoice(invoice=None, data=None):
    ...
    try:
        ...
        # Store original payments to preserve them
        original_invoice_payments = invoice.get('payments', [])
        ...
        # Restore payments if ERPNext reset them (handle both regular and return invoices)
        payments_restored = False
        if original_invoice_payments and invoice_doc.get('payments'):
            for i, payment in enumerate(invoice_doc.payments):
                if i < len(original_invoice_payments):
        ...
        if doctype == "Sales Invoice":
            try:
                from pos_next.api.sales_order_status import update_sales_order_status_after_invoice
                status_update = update_sales_order_status_after_invoice(invoice_doc.name)
                if status_update.get("success") and status_update.get("updated_orders"):
                    # Log successful status updates
                    updated_count = len(status_update['updated_orders'])
                    frappe.log_error(
                        f"Updated {updated_count} sales order(s) for invoice {invoice_doc.name}",
                        "Sales Order Status Update"
                    )
            except Exception as status_error:
                # Don't fail the invoice submission, just log the error
                frappe.log_error(
                    f"Failed to update sales order status for invoice {invoice_doc.name}: {str(status_error)[:100]}",
