...
# --- RLS1100C / Scale barcode (EAN-13 weight) support ---
# Format: 2-digit prefix + 5-digit PLU + 5-digit weight (grams) + 1 check digit = 13 digits
# RLS1100C can use prefix "02" or "21" (e.g. scanned 2112311000728 = prefix 21, PLU 12311, weight 72g).
# Setup: For each scale item, add one row in Item Barcode with barcode = base barcode
#        (same prefix + PLU as on scale + "00000" + EAN-13 check digit). Example: PLU 12311, prefix 21
#        -> base = "211231100000" + check digit -> store that in Item Barcode for the item.
SCALE_BARCODE_PREFIXES = ("02", "21")  # RLS1100C: 02 or 21
SCALE_BARCODE_LEN = 13
SCALE_PLU_START = 2   # 0-based (after 2-digit prefix)
SCALE_PLU_LEN = 5
SCALE_WEIGHT_START = 7  # 0-based
SCALE_WEIGHT_LEN = 5


def _ean13_check_digit(first_12):
	"""Compute EAN-13 check digit for first 12 digits (string or list of ints)."""
	s = 0
	for i, c in enumerate(first_12[:12]):
		d = int(c)
		# from right: positions 12,10,8,... (0-based 11,9,7,...) multiply by 1
		if (11 - i) % 2 == 0:
			s += d
		else:
			s += d * 3
...
def _parse_scale_barcode(barcode):
	"""
	Parse RLS1100C-style scale barcode (EAN-13 with weight).
	Supports prefixes 02 and 21. Returns dict with plu, weight_grams, base_barcode or None if not valid.
	"""
	barcode = (barcode or "").strip()
	if len(barcode) != SCALE_BARCODE_LEN or not barcode.isdigit():
		return None
	prefix = next((p for p in SCALE_BARCODE_PREFIXES if barcode.startswith(p)), None)
	if not prefix:
		return None
	plu = barcode[SCALE_PLU_START : SCALE_PLU_START + SCALE_PLU_LEN]
	weight_str = barcode[SCALE_WEIGHT_START : SCALE_WEIGHT_START + SCALE_WEIGHT_LEN]
	weight_grams = flt(weight_str, 0)
	# Base barcode for lookup: same prefix + PLU + 00000 + check digit
	base_12 = prefix + plu + "0" * SCALE_WEIGHT_LEN
	check = _ean13_check_digit(base_12)
	base_barcode = base_12 + str(check)
	return {
		"plu": plu,
...
def search_by_barcode(barcode, pos_profile, price_list=None):
	...
	try:
		...
		barcode = (barcode or "").strip()
		scale_qty = None  # Set when item is found via scale barcode

		# 1) Exact match in Item Barcode or Item code
		...
		if barcode_data:
			...
			# 2) Try scale barcode (RLS1100C: prefix + PLU + weight + check)
			parsed = _parse_scale_barcode(barcode)
			if parsed:
				base_barcode = parsed["base_barcode"]
				plu = parsed["plu"]
				scale_qty = flt(parsed["weight_grams"], 0) / 1000.0  # kg
				barcode_data = frappe.db.get_value(
					"Item Barcode", {"barcode": base_barcode}, ["parent", "uom"], as_dict=True
				)
				if not barcode_data:
					# Fallback: find item by PLU only (user can add barcode "12311" for scale PLU 12311)
					barcode_data = frappe.db.get_value(
						"Item Barcode", {"barcode": plu}, ["parent", "uom"], as_dict=True
					)
				if barcode_data:
					item_code = barcode_data.parent
					barcode_uom = barcode_data.uom
		...
		# When from scale barcode (RLS1100C), set qty to parsed weight (kg)
		if scale_qty is not None and scale_qty > 0:
			item_details["qty"] = scale_qty
